...
bool isvalid(const char *team) {

    return true;
  }

char *Team(const char *team) {

  if (isvalid (team) == false)
   return "Illegal";

     char *result;
     if (team == Arsenal) result = "In group stages: 1st in group A";
...
void test() {
    ...
    //n = eq(n, Team("Arsenal"), "In group stages: 1st in group A");
...
int main(int argc, char **argv) {
    if (argc == 1) {
        ...
        char *result = Team(argv[1]);
        printf("%s\n", result);
    }
    else {
        fprintf(stderr, "Use e.g.: ./ChampionsLeague Arsenal\n");
        exit(1);
    }
}
